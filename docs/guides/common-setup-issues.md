...
For detectable setup issues we try to log information in the `console` for `development` builds of `vue-beautiful-dnd`. If things are not working, first thing to do is check your `console`.
...
We go over this in our [Get started with `vue-beautiful-dnd` course](https://egghead.io/lessons/react-move-items-between-columns-with-vue-beautiful-dnd-using-ondragend)
